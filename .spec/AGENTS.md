...
**Lumio Image Studio** —— Next.js 全栈图像生成公开站,前端中文创作工作台,服务端负责生成代理、业务数据、图片存储、免费试用与基础防刷。

- **技术栈:** Next.js(App Router)+ React + TypeScript(strict);PostgreSQL 业务数据(`pg`,`scripts/migrate.mjs` 应用 `src/server/db/schema.sql`);S3/R2 图片存储(`@aws-sdk/client-s3`);请求校验用 Zod;JWT 用 `jose`;测试 Vitest。部署 Zeabur。
- **目录分层:** `src/app/`(App Router 页面 + `api/` 路由 + `admin/` 后台 + `share/`);`src/components/`(客户端 UI,图像工作台);`src/server/`(仅服务端:auth/security、providers、storage、generation、canvas、db、domain、admin、sub2api);`src/server/domain/*.test.ts`(核心业务规则单测);`scripts/`(运维脚本);`canvas-app/`、`docs/`、`devDoc/`。
- **服务商适配:** Gemini 与 GPT Image 两路图像生成的服务端适配(`src/server/providers/`)。
- **关键边界(单一权威):** 本站只负责**图像生成 + 新设备/新用户共 3 次本地免费试用 + 提示词/任务/历史/资产元数据 + 上传与结果保存**。**注册登录、注册/邀请赠送次数、支付、平台 Token/API Key 与计费策略归 `https://api.lumio.games/`**;本站免费额度用尽后,登录用户改用 Lumio 账户的图片生成 Key/余额。2K/4K 仅走登录 Lumio 账户,本地免费仅支持 1K。
- **安全面(改动经过必至少快审):** JWT 校验(`src/server/security/`)、鉴权、`/admin` 后台、生成成本控制路径(匿名与登录两条生成链路都要校验,直接影响成本)、可执行配置(hooks)。
...
- **收口门槛:** `npm test`(Vitest 全量单测)+ `npm run build`(Next.js 生产构建 + 类型检查);交付前必须通过。改动触碰路由 / React 组件 / 共享类型时 `npm run build` 尤为必要。**改动涉及 `.spec/` 时**额外跑 `node .spec/tools/spec-lint.mjs`(提交前由 `.claude/settings.json` hook 机器兜底)。
