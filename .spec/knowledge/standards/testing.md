...
- **框架：** Vitest（`vitest.config.ts`）。单测**紧贴被测代码**放置，命名 `*.test.ts`（如 `src/server/domain/*.test.ts`）。
- **命令：** `npm test`（`vitest run`，一次性）/ `npm run test:watch`（watch）。收口门槛见 `AGENTS.md`。
- **测试重点：** 配额计算、邀请奖励、模型输入归一化、provider 适配器、API 边界情形；匿名与登录两条生成链路都要覆盖(直接影响成本控制)。
- **构建即类型检查：** 触碰路由、React 组件或共享类型时，交付前另跑 `npm run build`。
- 集成/E2E（真库 / 真 S3 / 真 provider）不进默认 `npm test`，显式触发以保持收口快。
