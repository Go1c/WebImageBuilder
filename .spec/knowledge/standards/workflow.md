...
feature 分支 + PR 合入 `main`（当前活跃分支如 `feat/admin-backend`）。不直接在 `main` 上开发。
...
- **PR 必含：** 简短 Summary、改动清单、**验证证据**（`npm test` / `npm run build` 命令与关键输出，不止「已通过」）、known gaps；改动涉及部署时列出所需环境变量；UI 改动附截图 / 短录屏。
- **合并方式：** PR 合入 `main`；push 共享分支 / 开公开 PR 属对外发布动作，执行前需用户确认（见 `rules/system.md`）。
- 提交信息用 Conventional Commits（见下「提交规范」）。
